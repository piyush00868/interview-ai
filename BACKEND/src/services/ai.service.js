...
async function generateInterviewReport({ resume, selfDescription, jobDescription }) {
    ...
    const prompt = `Generate a comprehensive interview report for a candidate applying for a job. Use the following details:

Resume: ${resume}
Self Description: ${selfDescription}
Job Description: ${jobDescription}

First, extract the job title from the job description and set it as the 'title' field. 
Return ALL fields from the JSON schema and ensure arrays are non-empty.
`
    ...
    const parsedResult = interviewReportSchema.safeParse(JSON.parse(response.text || "{}"))
    if (!parsedResult.success) {
        throw new Error(`AI returned invalid interview report format: ${parsedResult.error.message}`)
    }
